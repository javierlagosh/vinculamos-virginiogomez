...
class UsuariosController extends Controller
{
    ...
    {
        $rolesPermitidos = ['Administrador', 'Supervisor', 'Digitador', 'Observador'];

        $usuarios = Usuarios::join('role_usuarios', 'role_usuarios.rous_codigo', 'usuarios.rous_codigo')
            ->whereIn('role_usuarios.rous_nombre', $rolesPermitidos)
            ->select(
            'usuarios.usua_nickname',
            'role_usuarios.rous_nombre',
            'usuarios.usua_nombre',
            'usuarios.usua_apellido',
            'usuarios.usua_nickname',
            'usuarios.usua_email',
            'usuarios.usua_creado',
            'usuarios.usua_vigente',
            )->get();
        $roles = RoleUsuarios::whereIn('rous_nombre', $rolesPermitidos)->get();
        return view('admin.usuarios.gestion', compact('usuarios', 'roles'));
    }
    ...
    public function crearUsuario(Request $request) {
        ...
        $usuaVerificar = Usuarios::where(['usua_nickname' => $request->nickname])->first();
        ...
        return redirect()->route('admin.listar.usuarios')->with('exitoRegistro', 'El usuario fue registrado correctamente.');
    }

    public function habilitarUsuario($usua_nickname) {
        $usuaVerificar = Usuarios::where(['usua_nickname' => $usua_nickname])->first();
        ...
        return redirect()->route('admin.listar.usuarios')->with('exitoUsuario', 'El usuario fue habilitado correctamente.');
    }

    public function deshabilitarUsuario($usua_nickname) {
        $usuaVerificar = Usuarios::where(['usua_nickname' => $usua_nickname])->first();
        if (!$usuaVerificar) return redirect()->back()->with('errorUsuario', 'El usuario no se encuentra registrado.');
        ...
        return redirect()->route('admin.listar.usuarios')->with('exitoUsuario', 'El usuario fue deshabilitado correctamente.');
    }
    ...
    public function editarUsuario(Request $request, $usua_nickname) {
        ...
        return redirect()->route('admin.listar.usuarios')->with('exitoUsuario', 'Los datos del usuario fueron actualizados correctamente.');
    }
    ...
    public function actualizarClaveUsuario(Request $request, $usua_nickname) {
        ...
        return redirect()->route('admin.listar.usuarios', $usua_nickname)->with('exitoClave', 'La contraseña del usuario fue actualizada correctamente.');
    }
